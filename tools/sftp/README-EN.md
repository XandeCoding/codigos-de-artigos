...
Well, in case you don't know the **SFTP** protocol (_Secure File Transfer Protocol_ or _SSH File Transfer Protocol_), it is a network file transfer protocol like FTP, but it uses SSH to encrypt commands and data (during transmission).
...
2 - In my case, the user is _alexandre_. Now let's check the user's ID:
...
3 - Do you see the `uid` value preceding our username in parentheses? The value inside it is what we want. In my case, it's _1000_. We'll need this when we create the users with access to SFTP.

4 - Add the necessary directories to the group using the _chown_ command on the folder we are working in, with the user and group in the following format: _user:group_.

```
chown -R alexandre:alexandre ./sftp
```

5 - Adding permissions: the creator of the file and the group can _read_, _edit_, and _delete_ data, while others can only execute read operations (774).
...
Just create a file named _users.conf_ and add the user information in the following format `username:password:uid`. I will explain each field:

- **username:** Can be any username, including your current user (my case).
- **password:** I will leave it blank because we'll use an _SSH_ key to authenticate.
- **uid:** The UID we got in the previous step _(if you're in a hurry, try 1000 or 1001)_.
...
To generate the keys, I will use _ssh-keygen_, which usually comes installed on Ubuntu.

1 - Enter the _creds_ directory inside _/sftp_.

2 - Use the command **ssh-keygen -t rsa** and type the filename. It can be any name; I used _key_.
...
1 - Open the _Site Manager_ in the File tab:

![file/site manager]({{ get_img_url('tools/sftp/media/guia_arquivo_en.png') }})

2 - Follow the suggested config:

![site manager/config]({{ get_img_url('tools/sftp/media/config_en.png') }})

- **Protocol:** SFTP
- **Host:** localhost
- **Port:** 2222 - Normally port 22 is used, but I redirected it to 2222 to avoid conflicts with other ports.
- **Logon Type:** Key file
- **User:** alexandre - Add the user from the _users.conf_ file.
